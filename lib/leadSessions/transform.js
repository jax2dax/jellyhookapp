...
/**
 * Reconstructs a minimal scroll trace from the 4 checkpoint columns that
 * reproduces the exact "seen once" / "seen more than once" geometry when fed
 * through framePlate's deriveVisitGeometry (bin-counting) — see
 * framePlate/geometry/deriveVisitGeometry.ts. Traced by hand against several
 * multi-phase scroll patterns; the shallowest 2x-crossed bin this produces
 * always matches revisitStartDepth's own documented definition.
 */
export function buildScrollTrace(pageViewRow) {
  ...
  if (revisit !== null) trace.push({ t: 2, y: revisit });
  trace.push({ t: 3, y: exit });
  return trace;
}
